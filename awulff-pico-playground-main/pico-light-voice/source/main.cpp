...
int main() {
  ...
  gpio_disable_pulls(
      26 + CAPTURE_CHANNEL); // RP2350-E9: disable pull resistors on ADC pin
  adc_init();
  adc_select_input(CAPTURE_CHANNEL);
  adc_fifo_setup(true,  // Write conversions to the sample FIFO
  ...
  while (true) {
    ...
    for (uint32_t i = 0; i < INSIZE; i++) {
      if (intermediate_buf[i] > max)
        max = intermediate_buf[i];
      if (intermediate_buf[i] < min)
        min = intermediate_buf[i];
    }

    for (uint32_t i = 0; i < INSIZE; i++) {
      float val = ((float)intermediate_buf[i] - (float)min) /
                      ((float)max - (float)min) * 2 -
                  1;
      val = val * 32766;
      features[i] = val;
    }
    ...
    // --- Serial Debug: print all label scores ---
    printf("[Inference] ");
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
      printf("%s: %.2f  ", result.classification[ix].label,
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
      ...
      // "activate" turns lights ON and cycles modes
      if (strcmp(label, "activate") == 0 && score > thresh &&
          time_us_64() - last_activate_time > COOLDOWN_US) {
        printf(">>> DETECTED: ACTIVATE (score=%.2f) <<<\n", score);
        last_activate_time = time_us_64();
        // Set the result to 1 for the keyword you use to turn the lights on
        model_result = 1;
      }

      // "cancel" turns lights OFF
      if (strcmp(label, "cancel") == 0 && score > thresh) {
        printf(">>> DETECTED: CANCEL (score=%.2f) <<<\n", score);
